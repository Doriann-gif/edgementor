...
const DISCORD_GROUPS = [
  ...
  { name: "ICT Concepts Lab", members: 4120, description: "Study group for smart money concepts. Daily markup reviews and backtesting challenges.", tags: ["Forex", "ICT"], active: true },
  { name: "Crypto Swing Traders", members: 1870, description: "On-chain analysis meets technical analysis. BTC & ETH focused with altcoin screening channels.", tags: ["Crypto", "Supply & Demand"], active: false },
  { name: "Price Action Purists", members: 3050, description: "No indicators, no noise. Clean charts and high-probability setups across all markets.", tags: ["Equities", "Price Action"], active: true },
];
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative">
        ...
        <section className="py-20 sm:py-28 px-4 sm:px-6">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <h1 className="font-heading text-4xl sm:text-5xl lg:text-6xl font-bold text-foreground tracking-tight leading-[1.1] mb-5">
              Learn from traders who <span className="text-primary">actually trade</span>
            </h1>
            ...
            <div className="flex items-center justify-center gap-3">
              <Link to="/mentors"><Button size="lg" className="h-12 px-8 font-semibold text-sm">Browse Mentors <ArrowRight className="h-4 w-4 ml-1" /></Button></Link>
              <Button variant="outline" size="lg" className="h-12 px-8 font-semibold text-sm">Watch Free Content</Button>
            </div>
          </div>
        </section>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {featuredMentors.map((mentor) => (
                <Link key={mentor.id} to={`/mentor/${mentor.id}`} className="group rounded-2xl border border-border bg-card p-5 transition-all duration-300 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5">
                  <div className="flex items-start gap-3 mb-4">
                    <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-primary/10 text-primary font-heading font-bold text-sm">{mentor.avatar}</div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {mentor.instruments.map((i) => <span key={i} className="rounded-md bg-primary/10 px-2 py-0.5 text-[11px] font-medium text-primary">{i}</span>)}
        ...
        <section className="py-16 sm:py-20 px-4 sm:px-6 bg-card/20">
          <div className="max-w-6xl mx-auto">
            <div className="mb-8">
              <div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-3 py-1 text-xs font-medium text-primary mb-3"><MessageCircle className="h-3.5 w-3.5" /> Community</div>
              <h2 className="font-heading text-2xl sm:text-3xl font-bold text-foreground tracking-tight">Discord Trading Groups</h2>
              <p className="text-sm text-muted-foreground mt-1.5">Join active communities of traders sharing setups, analysis, and ideas daily.</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {DISCORD_GROUPS.map((group) => (
                <div key={group.name} className="rounded-2xl border border-border bg-card p-5 transition-all duration-300 hover:border-primary/30">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          ...
                          {group.active && <span className="flex items-center gap-1 text-xs text-primary"><span className="h-1.5 w-1.5 rounded-full bg-primary animate-pulse" />Active now</span>}
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex flex-wrap gap-1.5">
                      {group.tags.map((tag) => <span key={tag} className="rounded-md bg-secondary px-2 py-0.5 text-[11px] font-medium text-secondary-foreground">{tag}</span>)}
                    </div>
                    <Button variant="outline" size="sm" className="h-7 text-[11px]">Join</Button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <section className="py-16 sm:py-20 px-4 sm:px-6">
          <div className="max-w-6xl mx-auto">
            <div className="mb-8">
              <div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-3 py-1 text-xs font-medium text-primary mb-3"><Play className="h-3.5 w-3.5" /> Free Content</div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {VIDEOS.map((video) => (
                <div key={video.title} className="group rounded-2xl border border-border bg-card overflow-hidden transition-all duration-300 hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 cursor-pointer">
                  <div className="aspect-video bg-secondary flex items-center justify-center relative">
                    <span className="font-heading font-bold text-2xl text-muted-foreground/30">{video.thumbnail}</span>
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                      <div className="h-12 w-12 rounded-full bg-primary/90 flex items-center justify-center"><Play className="h-5 w-5 text-primary-foreground fill-current ml-0.5" /></div>
                    </div>
                    <span className="absolute bottom-2 right-2 bg-background/80 text-foreground text-[10px] font-medium px-1.5 py-0.5 rounded">{video.duration}</span>
                  </div>
                  <div className="p-4">
        ...
        <section className="py-16 sm:py-20 px-4 sm:px-6 bg-card/20">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
              {[
                { icon: Shield, title: "Verified Only", desc: "Every mentor provides proof of profitability. No fake gurus, no unproven strategies." },
                { icon: BarChart3, title: "Real Strategies", desc: "Learn proven methodologies — ICT, order flow, price action — from traders who use them daily." },
                { icon: Users, title: "Active Community", desc: "Join Discord groups with thousands of traders sharing setups, analysis, and support." },
              ].map((item) => (
                <div key={item.title} className="rounded-2xl border border-border bg-card p-6">
                  <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center mx-auto mb-4"><item.icon className="h-5 w-5 text-primary" /></div>
        ...
        <section className="py-20 sm:py-24 px-4 sm:px-6">
          <div className="max-w-3xl mx-auto text-center">
            <h2 className="font-heading text-3xl sm:text-4xl font-bold text-foreground tracking-tight mb-4">Ready to level up your trading?</h2>
            ...
            <div className="flex items-center justify-center gap-3">
              <Link to="/mentors"><Button size="lg" className="h-12 px-8 font-semibold text-sm">Find a Mentor <ArrowRight className="h-4 w-4 ml-1" /></Button></Link>
              <Link to="/apply"><Button variant="outline" size="lg" className="h-12 px-8 font-semibold text-sm">Apply as Mentor</Button></Link>
            </div>
          </div>
        </section>
